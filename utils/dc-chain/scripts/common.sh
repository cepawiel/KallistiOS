...
printarr() {
  ...
  local ver="$1_VER"
  printf "%s=\"%s\"\n" "${ver}" "${!ver}"
  local git_repo="$1_GIT_REPO"
  printf "%s=\"%s\"\n" "${git_repo}" "${!git_repo}"
  local git_branch="$1_GIT_BRANCH"
  printf "%s=\"%s\"\n" "${git_branch}" "${!git_branch}"
  local tarball_type="$1_TARBALL_TYPE"
  printf "%s=\"%s\"\n" "${tarball_type}" "${!tarball_type}"
  local tarball_url="$1_TARBALL_URL"
...
export KERNEL_MIRROR_URL=mirrors.kernel.org/gnu
...
printarr SH_BINUTILS
printarr SH_GCC
printarr NEWLIB
printarr GDB
printarr INSIGHT
printarr ARM_BINUTILS
printarr ARM_GCC
